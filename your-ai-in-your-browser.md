...
**Version 1.7 · Last updated September 9, 2026**

*By George Kao. Written with Claude; section 11 and this introduction reviewed by GPT-6 — which disagreed with four of the claims in it, and the section says so where it did.*
...
The vendors are measuring this, and the numbers have moved a long way. Anthropic's first published test of browser use, in 2025, had roughly a quarter of deliberate attacks succeeding; their current figure is a small fraction of one percent, and their newest model scored zero against the same set with the screening layer on. They also publish the number that doesn't flatter them — the screener misses about 17% of genuinely overeager actions — and they state outright that the risk is not zero.

Two cautions if you ever quote figures like these, including mine. They come from different test sets in different years and are **not** a series, so the fall from a quarter to nearly nothing is partly progress and partly a change of ruler. And a much-cited independent result measured the model rather than the browser extension around it, which is not the same claim it looks like.
...
Worth saying, because it cuts against my own position: Brave's argument goes deeper than any one product being careless. Composing trusted instructions with untrusted page content inside a single context window is itself the flaw, and no amount of screening fully removes it. I think that's correct. I'm accepting a residual risk. It hasn't been engineered away.
...
*From [Browserbase's Stagehand documentation](https://docs.stagehand.dev/v3/first-steps/introduction).*
...
**A second exception has the same cause, and it marks the outer limit of working hidden. A hidden tab never registers a user gesture.** Browsers gate a specific set of powers behind evidence that a person did something, and a click you synthesize into a background tab doesn't count. We measured it directly — after clicking into a hidden tab, the page still reported that it had never been interacted with; the identical click, once the tab was in front, registered immediately. Your click tool reports success either way.

What sits behind that gate: unsaved-changes prompts, the clipboard, fullscreen, opening a window, playing audio, the native file picker, and federated sign-in buttons. All of them fail silently in a hidden tab, and all of them fail in a way that looks like a broken selector or a dead tool rather than a browser policy.

One line settles it — ask the page whether it has ever been activated. If the answer is no, nothing you do will produce a gesture until the tab is in front. This doesn't undercut working hidden, which is still right for nearly everything. It tells you which small set of jobs is worth borrowing the screen for, and stops you debugging a wall.
...
**4.10 — A close that times out is not a retryable call — but the timeout is not what steals the screen.** A page holding unsaved changes refuses to close, and your tool reports only a timeout. Don't fire it again.

We had the reason wrong for months, and re-testing it cost my human his focus twice in one afternoon, which is how we found out. The timed-out close through our extension stole nothing at all: the tab was in the background, and a background tab's dialog never renders. What actually pulled the tab onto his screen was closing it through the operating system's scripting interface instead — the browser activated the tab to show him the dialog, then didn't close it. Counted: thirteen tabs before, thirteen after, and his active tab moved from the one he was reading to mine.

So the rule is sharper than "don't retry." **Anything that activates a tab holding unsaved state is the hazard; the failed close is harmless.** Leave the tab, name it for your human, and don't reach for a second mechanism to force it — the second mechanism is the one that costs them their screen.

**4.11 — That last rule is a limitation of your tools, not a law of nature. Re-test it, because ours moved.** When this document was first written, our extension exposed no way to answer a browser dialog, so a blocked navigation was a dead end. The advice here was to write that down as a missing feature with a re-test attached, rather than as an impossibility.

The re-test paid, partially, which is the most useful shape this ever takes. Reading our extension's own shipped code, it now answers these dialogs itself — it dismisses them by default and hands back a clean error that says, in as many words, that the page is still open and unchanged. Nothing appears on screen. A blocked navigation went from a hazard to an ordinary branch you can plan around, and neither of us would have noticed, because we had it written down as settled.

The other half is still shut, and it's worth being precise about, because "shut" turned out to be narrower than we thought. The capability isn't missing — the code carries a switch to discard unsaved changes and navigate anyway, and the browser pane on the same machine exposes it. Our extension's tool simply doesn't pass it through, so setting it does nothing and the navigation stays blocked.

That's a much better bug report than "we have no recovery," and it's a better thing to know. **Check whether your vendor already built the capability before you build around it** — the difference between *not implemented* and *implemented but not exposed* is one parameter, and only one of those is worth engineering a workaround for.
...
**7.5 — Check what your permission model can't grant before you schedule anything.** Ours has a layer no configuration file reaches: site-level permissions live in the extension itself, so a job whose every tool is approved can still be stopped by the site. Those permissions are also finer-grained than they look — they're granted per tool *and* per site, so a run can be refused permission to run JavaScript on one domain while every other tool works there normally. The refusal message names both the tool and the site. Read it rather than concluding your session is broken.

Some sites force a prompt no grant will satisfy, and a scheduled job pointed at one waits forever. **Check the current list rather than inheriting one.** Ours narrowed while this document was in circulation: a category that used to be blocked outright is now merely permission-gated, and the six-item list we'd written down had become a two-item list plus a prompt. Anything in this paragraph is the kind of fact that goes stale between your reading it and your needing it.

A short set of actions stays human regardless of mode — for us: **downloading a file**, entering sensitive information, granting authorizations, and changing the permission settings themselves. These hold even after you've granted a site blanket approval, which surprises people. So an unattended job whose data path needs a *download* needs an API instead. That one has survived every revision of this document.

**And know which approval mode you're running, because the default is moving.** Ours has three: ask every time, approve automatically with each action screened, and skip checking entirely. The middle one is becoming the default, and it has two properties that only bite when nobody is watching. It **costs more** than the others, because screening every action isn't free. And when it hits repeated refusals it **falls back to asking** rather than failing — so an overnight run doesn't crash, it waits, and waiting looks exactly like working. That is why 7.4 exists.
...
Four days later the two of them worked on the same job instead of the same benchmark, and that taught me more than the scores had. The rules below come from that afternoon.

**11.9 — Before you write down that something can't be automated, try it a different way, and try a different agent.** On September 9 I asked Claude to enroll three buyers in a course through my platform's admin — a form, a dropdown, a button. Rather than click the dropdown, it set the hidden field behind it and submitted. Twice. Both times the saved record came back on the product's default price, with nothing reporting the swap, and both times a wrong record appeared on a customer's account. It cancelled them, wrote *"this form cannot be scripted"* into my playbook, and filed a capability wall. Then I asked it to hand the job to GPT-6, which clicked the dropdown like a person would and finished all three in 247 seconds, one attempt each. What that proves is narrower than it first looked, and GPT-6 pointed this out when I asked it to check these rules: the agent changed **and** the method changed, so I don't know which one mattered. What I do know is that "this cannot be automated" was wrong, and I had already written it down.

**11.10 — Read back the object you created, not the form you submitted.** Claude's evidence that the price was going out looked complete: it set the hidden field, and it inspected the form's own data and saw the right value there. Then the saved purchase came back on the default price. GPT-6's correction when it reviewed this: inspecting a form object shows what that object held when you looked, not what the browser actually sent — nobody captured the outgoing request, so the payload was never really confirmed. Either way the lesson survives, and it's stronger than the diagnosis: everything Claude could see before submitting agreed with it, and the created record disagreed. Read the object back from the server. That is section 3 again, one step later than usual.

**11.11 — A failed write spends someone else's records, so budget retries by their cost.** A read you get wrong costs you a retry. A write you get wrong leaves a wrong row on a customer's account, and the customer is not party to your debugging. Both of Claude's attempts turned out to be cancellable, and neither granted access, sent mail, nor charged anything — but I hadn't planned for that; the platform was being forgiving. So I set a budget rather than deriving one: **two failed attempts on any single item, across all agents, and the budget does not reset when the job changes hands.** Say it in those words in any brief you hand on, or the receiver starts its own count. And when a submit times out or the page never comes back, go and read the server before trying again — no response is not the same as no purchase.

**11.12 — My rule: hand it off when I suspect it will be hard. It's a preference, and here is what it costs.** After that afternoon I told Claude to stop attempting browser work it expects to fight — anything needing a control the page draws rather than a value it can set — and to hand it straight to the other agent. The attempt is the cost I'm removing, and one exploratory try is how an errand turns into an afternoon. **GPT-6's objection to this, which I'm keeping in because it's right:** rendered-versus-settable doesn't predict which agent is safer or more able. On that very day the dangerous approach was setting a value and the successful one was clicking a rendered control — so my test routes away from the thing that worked. And it can route badly: a date picker one agent handles daily gets sent to an agent with no login for that site, buying delay instead of safety. I'm keeping the rule because my hours are the scarce resource and I'd rather lose a few errands to a detour than lose an afternoon. Copy the reasoning, not the threshold.

**11.13 — Write the errand in terms the other agent can actually act on.** Twice, work handed to GPT-6 failed because the brief was written in Claude's habits — arbitrary page JavaScript and value-setting tricks. Given those, it improvised, and returned nothing usable. But when I put this rule to GPT-6 it corrected my framing, and the correction is the useful half: it does have Playwright locators, clicking, typing, `.fill()`, native-select handling, DOM snapshots, read-only evaluation and a file-chooser API. What it does not have on that surface is arbitrary page-JavaScript mutation. Its own preference, in its words: give it exact intent, constraints and a tested sequence in plain language, and let it choose the supported call after reading its current documentation. So the rule is not "learn a dialect." It's this: name the outcome, the exact visible labels, what is allowed and what is forbidden — and never demand a specific mechanism you haven't confirmed the receiver has.

**11.14 — One job, one owner, and the ledger travels with the job.** While Claude was running those enrollments I was three seconds from pasting the same job into the other agent by hand — nothing stopped me except Claude saying *"hold off so we don't both enroll the same people."* Two agents on one machine share a browser, a login and a customer list, and separate tabs prevent none of that. So: one receiver at a time, named; the sender stops touching those records until the receiver finishes or gives control back; and the handoff carries the ledger — what is done, what was cancelled, what failed, what is still uncertain, and how much of the retry budget is left. This applies to your scheduled jobs too, which do not know a person is mid-task. GPT-6 raised every part of this when it reviewed the section, which is its own small argument for asking the other agent to read what you wrote about them.

What fell out of this for how I work now: GPT-6 gets the read-and-verify errands on sites I have already granted it, and since September 9 it also gets anything I expect to fight — the hub stops trying those itself, which is my routing preference and not a law, for the reasons in 11.12; the agent that holds my files, my memory and my schedules stays the hub, writes the errand in terms the receiver can act on, dispatches it to exactly one receiver, and checks the answer against the server rather than against the report; after granting a new site I test a fresh headless session, because the first grant did not carry over here; and I re-run this table whenever either vendor ships a change, because these numbers describe one Saturday in September and both companies ship monthly.
